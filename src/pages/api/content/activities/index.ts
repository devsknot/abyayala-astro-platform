// src/pages/api/content/activities/index.ts
// Astro API endpoint for managing activity logs.
...
// Handle GET requests (fetch activities with filters)
...
// Handle POST requests (create activity log)
...
async function verifyAuthentication(request: Request, env: any) {
    ...
    // GET is allowed without auth for activities
...
async function handleGetActivities(db: any, limit: number, offset: number, entityType: string | null, type: string | null, headers: HeadersInit) {
    try {
        let query = `
        ...
        const processedActivities = results.map((activity: any) => {
            if (activity.details) {
                try {
...
async function handleCreateActivity(activityData: any, db: any, headers: HeadersInit) {
...
// Helper function for relative time (copied from original)
